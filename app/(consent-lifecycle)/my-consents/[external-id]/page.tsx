...
export default function MyConsentsPage() {
  ...
  const externalIdFromUrl = params["external-id"] as string | undefined;

  const [withdrawDialogOpen, setWithdrawDialogOpen] = useState(false);
  const [renewDialogOpen, setRenewDialogOpen] = useState(false);
  const [selectedArtifactId, setSelectedArtifactId] = useState<string | null>(null);

  // Use URL param first, then fall back to localStorage
  const [userId] = useState<string>(() => {
    // Priority 1: Use external-id from URL params if available
